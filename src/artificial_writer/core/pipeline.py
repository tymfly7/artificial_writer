...
class Pipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # When no fetcher is injected, dispatch through the registry (which picks
        # HTML / PDF / YouTube by inspecting the URL).
        self._fetcher = fetcher
        self._summarizer = summarizer or build_summarizer(self._settings)
        self._storage = storage or Storage()

    def _fetch(self, url: str) -> FetchedArticle:
        if self._fetcher is not None:
            return self._fetcher.fetch(url)
        return registry.fetch(url, timeout=self._settings.request_timeout)
